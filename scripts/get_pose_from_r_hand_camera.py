...
def cb(req):  
    ...
    if req.flag is True:
        global pose_sum
        rospy.loginfo("get in aruco_marker_583_pick_frame cb")

        #ipdb.set_trace()
        for i in range (10): 
            try:

                (trans,rot) = listener.lookupTransform("/base", "/aruco_marker_583_pick_frame", rospy.Time())
        ...
        print("get marker pose from right hand camera", resp.pose)
        return resp

def main():
    global listener
    rospy.init_node('get_marker_pose_from_r_hand_camera')
    listener = tf.TransformListener()
    listener.waitForTransform("/base", "/aruco_marker_583_pick_frame", rospy.Time(), rospy.Duration(4.0))
    rospy.Service('get_pose_from_r_hand_camera', get_pose , cb)
    print "get_pose_from_right_hand_camera."
    rospy.spin()
